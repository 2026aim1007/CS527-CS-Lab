...
void execute(void) {
    ...
    int32_t op2_val = ((opcode >= 0x09 && opcode <= 0x0F) || (opcode >= 0x29 && opcode <= 0x2B)) ? src2 : Register[src2];
    ...
    if (opcode == 0x01 || opcode == 0x09) { // ADD
    ...
    else if (opcode == 0x05 || opcode == 0x0D) { // Scalar Read
    ...
    else if (opcode == 0x06 || opcode == 0x0E) { // Scalar Write
    ...
    else if (opcode >= 0x21 && opcode <= 0x33) {
        int32_t result[8];
        for(int i = 0; i < 8; i++) {
            int32_t v1 = VectorRegister[src1][i];
            int32_t v2;
            if (opcode >= 0x21 && opcode <= 0x23) v2 = VectorRegister[src2][i]; // V op V
            else if (opcode >= 0x31 && opcode <= 0x33) v2 = Register[src2];     // V op S (Custom)
            else v2 = src2;                                                     // V op C
            int math_type = opcode % 0x10;
            if (math_type == 1 || math_type == 9) { // Add
                result[i] = v1 + v2;
            } else if (math_type == 2 || math_type == 0xA) { // Sub
                result[i] = v1 - v2;
            } else { // Mul
                result[i] = v1 * v2;
            }
        ...
        for(int i = 0; i < 8; i++) {
            VectorRegister[dest][i] = result[i];
        }
    }
    else if (opcode == 0x25 || opcode == 0x2C) { // Vector Memory Read
        int32_t base_addr = (opcode == 0x25) ? Register[src2] : src2;
        for (int i = 0; i < 8; i++) {
            int32_t addr = base_addr + (i * 4);
            if (addr >= 0 && addr <= 4088) {
                VectorRegister[dest][i] = (unsigned char)Data[addr] | 
                                          ((unsigned char)Data[addr+1] << 8) | 
                                          ((unsigned char)Data[addr+2] << 16) | 
                                          ((unsigned char)Data[addr+3] << 24);
            }
        }
        if (opcode == 0x25) Register[src2] += 32; 
    }
    else if (opcode == 0x26 || opcode == 0x2E) { // Vector Memory Write
        int32_t base_addr = (opcode == 0x26) ? Register[dest] : dest;
        for (int i = 0; i < 8; i++) {
            int32_t addr = base_addr + (i * 4);
            int32_t val = VectorRegister[src2][i];
            if (addr >= 0 && addr <= 4088) {
                Data[addr]   = val & 0xFF;
                Data[addr+1] = (val >> 8) & 0xFF;
                Data[addr+2] = (val >> 16) & 0xFF;
        ...
        if (opcode == 0x26) Register[dest] += 32; 
    }
    else if (opcode == 0x27) { // Vector Move
        for(int i = 0; i < 8; i++) VectorRegister[dest][i] = VectorRegister[src2][i];
    }
}