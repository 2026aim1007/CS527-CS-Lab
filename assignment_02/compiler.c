...
void compile(const char* filename) {
    ...
    if (!in || !out) {
        printf("Error: Could not open input file or program.byte\n");
        exit(1);
    }
    char line[256];
    int ins_count = 0;
    ...
    while (fgets(line, sizeof(line), in)) {
        ...
        int reg, addr;
        if (sscanf(p, "Read x%d, %d", &reg, &addr) == 2) {
            fprintf(out, "%02X %02X 00 %02X\n", 0x0D, reg, addr);
            current_index++; continue;
        }
        if (sscanf(p, "Write x%d, %d", &reg, &addr) == 2) {
            fprintf(out, "%02X %02X 00 %02X\n", 0x0E, addr, reg);
            current_index++; continue;
        }
        char lhs[64] = {0}, rhs[64] = {0};
        if (sscanf(p, " %[^=] = %[^\n]", lhs, rhs) == 2) {
            int d_reg=-1, d_mem_reg=-1, d_mem_const=-1;
            sscanf(lhs, " [x%d]", &d_mem_reg);
            sscanf(lhs, " [%d]", &d_mem_const);
            sscanf(lhs, " x%d", &d_reg);
            ...
            if (sscanf(rhs, " x%d %c x%d", &s1_reg, &op_char, &s2_reg) == 3) {
                ...
                if (d_mem_reg != -1) fprintf(out, "%02X %02X 00 %02X\n", 0x06, d_mem_reg, s2_reg);
                else if (d_mem_const != -1) fprintf(out, "%02X %02X 00 %02X\n", 0x0E, d_mem_const, s2_reg);
                else fprintf(out, "%02X %02X 00 %02X\n", 0x07, d_reg, s2_reg); 
            }
